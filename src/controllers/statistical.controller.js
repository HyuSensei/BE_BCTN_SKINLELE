...
export const getStatisticalDoctor = async (req, res) => {
  try {
    ...
    const conversionRate = totalStats.completed > 0 
    ...
    return res.status(200).json({
      ...
      data: {
        totalStats: {
          ...
          revenueRate: totalStats.totalSales > 0 
            ? `${((totalStats.revenue / totalStats.totalSales) * 100).toFixed(1)}%`
            : '0%', 
        },
        averageRating: avgRating[0]?.averageRating?.toFixed(1) || 0,
        totalReviews: avgRating[0]?.totalReviews || 0,
        stats: dailyStats.map(day => ({
          ...day,
          conversionRate: day.totalBookings > 0
            ? `${((day.completed / day.totalBookings) * 100).toFixed(1)}%`
            : '0%',
          revenueRate: day.totalSales > 0
